...
class CustomBertModel(nn.Module, ABC):
    def __init__(self, args):
        ...
        with open("data/WN18RR/entities.json", "r") as file:
        ...
        for item in self.entity_data:
            ...
            self.new_dict1[entity_name] = [entity_id, entity_desc]
    ...
    def compute_logits(self, epoch, output_dict: dict, batch_dict: dict) -> dict:
        ...
        for i in range(batch_size):
            ...
            if len(entity_list) > 10:
                ...
                '''array = np.array(third_elements)
                n = len(third_elements)
                distance_matrix = np.zeros((n, n))
                for i in range(n):
                    for j in range(n):
                        distance_matrix[i][j] = np.dot(array[i], array[j])
                n = len(third_elements)
                distance_matrix = np.zeros((n, n))
                ...
                for i in range(n):
                    for j in range(n):
                        distance_matrix[i][j] = np.sum(
                            np.abs(np.array(third_elements[i]) - np.array(third_elements[j])))
                n = len(third_elements)
                distance_matrix = np.zeros((n, n))

                for i in range(n):
                    for j in range(n):
                        distance_matrix[i][j] = np.max(
                            np.abs(np.array(third_elements[i]) - np.array(third_elements[j])))'''
